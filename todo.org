...
* Current
- [X] basic triangle example
  - [X] conditionally remove vertex/index buffers
    - [X] fix template formatting
    - [X] make renderer coompile with empty resources
    - [X] make the example runnable

- [X] delete files in generated module on regen
- [X] delete json & spv on regen

- [X] track window aspect ratio in renderer, not game
  have App handle resize

- [ ] update naming convention in shader atlas entries
  remove stutter prefix
...
- [ ] move to using a cargo workspace
  - new git repo
  - plan for supporting roc
