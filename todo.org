...
* Current
- [X] improve generated file tests
  - [X] make durable to order changes
  - [X] include shader json
  - [X] try to use a more real code path
...
- [ ] signed distance fields / animated rick example
  ...
  - [ ] avoid requiring a uniform buffer (from ParameterBlock)
  - [ ] avoid requiring a ParameterBlock at all
  - [ ] avoid requiring vertex & index buffers if possible
    will this example end up needing them?
