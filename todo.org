...
- [ ] try to use the reflection-based vk pipeline layout in the renderer
  ...
  - [ ] first compare the outputs w/debug
    need to create either the vulkan resources or a recipe for them ahead of time
    can't hold onto the slang handles after session is dropped
